...
function Appointments() {
  ...
  return <div className="content-page"><div className="page-title"><div><span className="eyebrow">Care planning</span><h2>Appointments</h2><p>Book and manage your visits with your care team.</p></div></div><div className="content-columns"><form className="dashboard-card form-card" onSubmit={book}><h3>Book a visit</h3><label>Clinician<select value={doctorId} onChange={event => setDoctorId(event.target.value)} required><option value="">Choose a clinician</option>{doctors.map(doctor => <option key={doctor.id} value={doctor.id}>{doctor.name} {doctor.specialty ? `(${doctor.specialty})` : ''}</option>)}</select></label><label>Date<input type="date" value={date} onChange={event => setDate(event.target.value)} required /></label><label>Time<select value={slot} onChange={event => setSlot(event.target.value)}><option>10:00 AM</option><option>2:00 PM</option><option>4:30 PM</option></select></label><label>Reason / Notes <span className="optional">(optional)</span><input value={notes} onChange={event => setNotes(event.target.value)} placeholder="e.g. Check blood work, follow-up" /></label><ErrorMessage error={error} />{notice && <p className="success-message">{notice}</p>}<button className="button" type="submit">Request appointment <ArrowRight size={15} /></button></form><section className="dashboard-card list-card"><div className="card-title"><h3>Your appointments</h3><span>{appointments.length} total</span></div>{appointments.map(item => {
    const doc = typeof item.doctorId === 'object' && item.doctorId ? item.doctorId : doctors.find(d => d.id === item.doctorId)
    return <div className="list-row" key={item.id}><div><strong>{doc ? doc.name : 'Care team member'}</strong><small>{item.date} · {item.slot}{item.notes ? ` · ${item.notes}` : ''}</small></div><button className="text-button" onClick={() => cancel(item.id)} disabled={item.status !== 'scheduled'}>{item.status === 'scheduled' ? 'Cancel' : item.status}</button></div>
  })}{appointments.length === 0 && <p className="empty-state">No appointments yet.</p>}</section></div></div>
}
...
function Records() {
  ...
  const [form, setForm] = useState({ title: '', reportType: 'Lab result', fileUrl: '', summary: '' })
  const [error, setError] = useState('')
      ...
      {filtered.map(item => (
        <div className={`notification-item ${item.read ? '' : 'unread'}`} key={item.id} onClick={() => markRead(item.id)}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#566961' }}>{item.message || item.body}</p>
          </div>
        </div>
      ))}
